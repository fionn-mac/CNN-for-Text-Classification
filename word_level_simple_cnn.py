...
if __name__ == "__main__":
    ...
    for index, sentence in enumerate(x_train):
        ...
        words = [s if s in net.word_to_ix else UNK for s in words]
        x_train[index] = words
    ...
    for index, list_of_words in enumerate(x_train):
        list_of_words.extend([UNK] * (max_words - len(list_of_words)))
        assert len(list_of_words) == max_words
        x_train[index] = [net.word_to_ix[word] for word in list_of_words]

    print "train set done"

    for index, list_of_words in enumerate(x_test):
        list_of_words.extend([UNK] * (max_words - len(list_of_words)))
        assert len(list_of_words) == max_words
        x_test[index] = [net.word_to_ix[word] for word in list_of_words]
